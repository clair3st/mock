...
class TestTodos(object):
    """Test todos in class layout."""

    @classmethod
    def setup_class(cls):
        """Setup for testing env."""
    ...
    def test_getting_todos_when_response_is_ok(self):
        ...
        whenever the return_value is added to a mock, that mock is modified to
        be run as a function, and by default it returns another mock object.
        """
        todos = [{
            'userId': 1,
            'id': 1,
        ...
        # the mock should have a `json()` method that returns list of todos.
        self.mock_get.return_value.json.return_value = todos

        # Call the service, which will send a request to the server.
        response = get_todos()

        # If the request is sent successfully, expect response to be returned.
        assert response.json() == todos
...
class TestUncompletedTodos(object):
    ...
    def test_getting_uncompleted_todos_when_todos_is_not_none(self):
        ...
        # Confirm that the mock was called.
        assert self.mock_get_todos.called
